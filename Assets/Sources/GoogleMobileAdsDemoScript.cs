...
public class GoogleMobileAdsDemoScript : Singleton<GoogleMobileAdsDemoScript>
{
	...
	void SetID ()
	{
		...
		IDBanner = "ca-app-pub-7982045513389506/4895447470";
		IDFull = "/35883025/bha_480_320";
	}

	public void ShowFull ()
	{
		if (PlayerPrefs.GetInt ("BuyItem") == 1) {
			return;
		}
		DateTime timeCurrent = System.DateTime.Now;
		if (!PlayerPrefs.HasKey ("AdmobFull")) {
			timeStart = timeCurrent.AddHours (-1);
			PlayerPrefs.SetString ("AdmobFull", timeStart.ToString ());
			PlayerPrefs.Save ();
		} else {
			timeStart = DateTime.Parse (PlayerPrefs.GetString ("AdmobFull"));
		}
	
		TimeSpan timespan = timeCurrent.Subtract (timeStart);
		float tmp = timespan.Days * 24 * 60 * 60 + timespan.Hours * 60 * 60 + timespan.Minutes * 60 + timespan.Seconds;
		if (tmp < 180) {
			return;
		}
		PlayerPrefs.SetString ("AdmobFull", DateTime.Now.ToString ());
		PlayerPrefs.Save ();
		ShowInterstitial ();
	}
	...
	{
		bannerView = new BannerView (IDBanner, AdSize.SmartBanner, AdPosition.Top);
	...
	{
		interstitial = new InterstitialAd (IDFull);
		// Register for ad events.
	...
	private void RequestRewardBasedVideo ()
	{
		...
		#elif UNITY_ANDROID
		string adUnitId = "ca-app-pub-5118048189730927/8771927893";
